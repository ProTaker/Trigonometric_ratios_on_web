...
def main():
    ...
    while True:
        while True:
            try:
                mode = int(input("モードを選択してください（1: 度数法[°], 2: 弧度法[rad]）: "))
                if mode == 1 or mode == 2:
                    break
                else:
        ...
        for question_number in range(1, 11):
            ...
            if options[answer - 1] == correct_answer:
                score += 1
                correct_answers.append(f"問題{question_number} : 正解！")
            else:
                correct_answers.append(
                    f"問題{question_number} : 間違いです。正解は {func}({angle}{unit})={correct_answer} "
                )
